...
//f Book-keeping routine to be called before adaptation
// Returns true if there is at least one new element generated by growth
template <int dimgrid, int dimworld>
  ...
  for (Iterator elem=this->leafbegin<0>(), end = this->leafend<0>(); elem != end; ++elem)
  {
    ...
    switch(mark)
    {
      ...
      case 1:
      {
        ...
        willGrow = willGrow || isAllowedToGrow;
      }
      case 2:
      {
        willGrow = true;
...
bool Dune::FoamGrid<dimgrid, dimworld>::grow()
{
  ...
  if(removedEntities)
  {
    eraseVanishedEntities(Dune::get<0>(entityImps_[maxLevel()]));
    eraseVanishedEntities(Dune::get<dimgrid>(entityImps_[maxLevel()]));
  }
...
bool Dune::FoamGrid<dimgrid, dimworld>::growSimplexElement(FoamGridEntityImp<dimgrid, dimgrid, dimworld>& element)
{
  ...
  dvverb << "Adding the new point: " << element.growthPoint_ << std::endl;
  Dune::get<0>(entityImps_[thisLevel]).push_back(FoamGridEntityImp<0, dimgrid, dimworld>(thisLevel,
...
bool Dune::FoamGrid<dimgrid, dimworld>::removeSimplexElement(FoamGridEntityImp<dimgrid, dimgrid, dimworld>& element)
{
  ...
  for(auto facet = element.facet_.begin(); facet != element.facet_.end(); ++facet)
  {
...
bool Dune::FoamGrid<dimgrid, dimworld>::mergeSimplexElement(FoamGridEntityImp<dimgrid, dimgrid, dimworld>& element)
{
  ...
  for (auto e = facet->elements_.begin(); e != facet->elements_.end(); ++e)
    for (auto ne = neighborFacet->elements_.begin(); ne != neighborFacet->elements_.end(); ++ne)
      if ((*e)==(*ne))
        return false;
